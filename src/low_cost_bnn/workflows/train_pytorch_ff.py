...
def parse_inputs():
    ...
    parser.add_argument('--rmse_weight', metavar='wgt', type=float, nargs='*', default=None, help='Weight to apply to the root mean squared error loss term')
    parser.add_argument('--rrmse_weight', metavar='wgt', type=float, nargs='*', default=None, help='Weight to apply to the relative root mean squared error loss term')
...
def train_pytorch_feedforward_step(
    model,
    optimizer,
    loss_function,
    feature_batch,
    ...
    if training and verbosity >= 4:
        for ii in range(n_outputs):
            logger.debug(f'     Model output {ii}: {mean_outputs[0, ii].detach().cpu().numpy()}')

    batch_loss_targets = target_batch
    batch_loss_predictions = mean_outputs
    if n_outputs == 1:
    ...
    step_root_mean_square_loss = loss_function._calculate_root_mean_square_loss(batch_loss_targets, batch_loss_predictions)
    step_relative_root_mean_square_loss = loss_function._calculate_relative_root_mean_square_loss(batch_loss_targets, batch_loss_predictions)
    ...
    return (
        ...
        torch.reshape(step_root_mean_square_loss, shape=(-1, n_outputs)),
        torch.reshape(step_relative_root_mean_square_loss, shape=(-1, n_outputs))
    )


def train_pytorch_feedforward_epoch(
    model,
    optimizer,
    dataloader,
    loss_function,
    ...
    step_root_mean_square_losses = []
    step_relative_root_mean_square_losses = []
    ...
    for feature_batch, target_batch in dataloader:
        ...
        step_total_loss, step_regularization_loss, step_root_mean_square_loss, step_relative_root_mean_square_loss = train_pytorch_feedforward_step(
            model,
            optimizer,
            loss_function,
            feature_batch,
        ...
        step_root_mean_square_losses.append(torch.reshape(step_root_mean_square_loss, shape=(-1, n_outputs)))
        step_relative_root_mean_square_losses.append(torch.reshape(step_relative_root_mean_square_loss, shape=(-1, n_outputs)))

        if verbosity >= 3:
            if training:
                logger.debug(f'  - Batch {nn + 1}: total = {step_total_loss.detach().cpu().numpy():.3f}, reg = {step_regularization_loss.detach().cpu().numpy():.3f}')
                for ii in range(n_outputs):
                    logger.debug(f'     Output {ii}: rmse = {step_root_mean_square_loss.detach().cpu().numpy()[0, ii]:.3f}, rrmse = {step_relative_root_mean_square_loss.detach().cpu().numpy()[0, ii]:.3f}')
            else:
    ...
    epoch_root_mean_square_loss = torch.sum(torch.cat(step_root_mean_square_losses, dim=0), dim=0)
    epoch_relative_root_mean_square_loss = torch.sum(torch.cat(step_relative_root_mean_square_losses, dim=0), dim=0)
    ...
    return (
        ...
        epoch_root_mean_square_loss,
        epoch_relative_root_mean_square_loss
    )


def meter_pytorch_feedforward_epoch(
    model,
    inputs,
    targets,
    losses,
    ...
    total_loss, reg_loss, rmse_loss, rrmse_loss = losses

    total_loss = total_loss.detach().cpu()
    reg_loss = reg_loss.detach().cpu()
    rmse_loss = rmse_loss.detach().cpu()
    rrmse_loss = rrmse_loss.detach().cpu()
    ...
    loss_metrics = {
        'total': np.nan,
        'reg': np.nan,
        'rmse': [np.nan] * n_outputs,
        'rrmse': [np.nan] * n_outputs,
    }
    performance_metrics = {
...
def train_pytorch_feedforward(
    ...
    for epoch in range(max_epochs):
        ...
        train_losses = train_pytorch_feedforward_epoch(
            model,
            optimizer,
            train_loader,
            loss_function,
        ...
        train_metrics = meter_pytorch_feedforward_epoch(
            model,
            train_data[0],
            train_data[1],
            train_losses,
        ...
        rmse_train_list.append(train_metrics['rmse'])
        rrmse_train_list.append(train_metrics['rrmse'])
        ...
        valid_losses = train_pytorch_feedforward_epoch(
            model,
            optimizer,
            valid_loader,
            loss_function,
        ...
        valid_metrics = meter_pytorch_feedforward_epoch(
            model,
            valid_data[0],
            valid_data[1],
            valid_losses,
        ...
        rmse_valid_list.append(valid_metrics['rmse'])
        rrmse_valid_list.append(valid_metrics['rrmse'])
        ...
        if (epoch + 1) % print_per_epochs == 0:
            ...
            for ii in range(n_outputs):
                logger.info(f'  -> Output {ii}: r2 = {r2_train_list[-1][ii]:.3f}, mse = {mse_train_list[-1][ii]:.3f}, mae = {mae_train_list[-1][ii]:.3f}, rmse = {rmse_train_list[-1][ii]:.3f}, rrmse = {rrmse_train_list[-1][ii]:.3f}')
            logger.info(f' {epoch_str} Valid -- total_valid = {total_valid_list[-1]:.3f}, reg_valid = {reg_valid_list[-1]:.3f}')
            for ii in range(n_outputs):
                logger.info(f'  -> Output {ii}: r2 = {r2_valid_list[-1][ii]:.3f}, mse = {mse_valid_list[-1][ii]:.3f}, mae = {mae_valid_list[-1][ii]:.3f}, rmse = {rmse_valid_list[-1][ii]:.3f}, rrmse = {rrmse_valid_list[-1][ii]:.3f}')
    ...
    metrics_dict = {
        ...
        'train_rmse': rmse_train_list[:last_index_to_keep],
        'train_rrmse': rrmse_train_list[:last_index_to_keep],
...
def launch_pytorch_pipeline_feedforward(
    data,
    input_vars,
    output_vars,
    ...
    root_mean_square_weights=None,
    relative_root_mean_square_weights=None,
    ...
    settings = {
        ...
        'root_mean_square_weights': root_mean_square_weights,
        'relative_root_mean_square_weights': relative_root_mean_square_weights,
    ...
    rmse_weights = [1.0] * n_outputs
    for ii in range(n_outputs):
        if isinstance(root_mean_square_weights, list):
            rmse_weights[ii] = root_mean_square_weights[ii] if ii < len(root_mean_square_weights) else root_mean_square_weights[-1]
    rrmse_weights = [1.0] * n_outputs
    for ii in range(n_outputs):
        if isinstance(relative_root_mean_square_weights, list):
            rrmse_weights[ii] = relative_root_mean_square_weights[ii] if ii < len(relative_root_mean_square_weights) else relative_root_mean_square_weights[-1]
    ...
    if save_initial_model:
        if checkpoint_path is not None and checkpoint_path.is_dir():
            ...
            checkpoint_path = None
    best_model, metrics = train_pytorch_feedforward(
        model,
        optimizer,
        features['train'],
        targets['train'],
...
def main():
    ...
    trained_model, metrics_dict = launch_pytorch_pipeline_feedforawrd(
        data=data,
        ...
        root_mean_square_weights=args.rmse_weight,
        relative_root_mean_square_weights=args.rrmse_weight,
